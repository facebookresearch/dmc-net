...
def train(train_loader, model, criterion, criterion_mse, optimizer, optimizer_gf, optimizer_d, epoch, cur_lr_cls, cur_lr_gf, cur_lr_d, lr_cls, lr_adv_g, lr_adv_d, lr_mse, att):
    # init meter
    ...
    top1 = AverageMeter()
    top5 = AverageMeter()
    acc_G_adv = AverageMeter()  # real/fake accuracy during training G
    acc_D_adv = AverageMeter()  # real/fake accuracy during training D
    ...
    for i, (input_flow, input_mv, input_residual, target) in enumerate(train_loader):
        ...
        valid_var = torch.cat([target_var.clone().fill_(1)]*args.num_segments, 0)
        valid_var.requires_grad = False
        fake_var = torch.cat([target_var.clone().fill_(0)]*args.num_segments, 0)
        ...
        if i % 2 == 0:
            ...
            # forward
            if att == 0:
                output, validity, gen_flow = model(input_mv_var, input_residual_var, input_flow_var)
            elif att == 1:
                output, validity, gen_flow, att_flow = model(input_mv_var, input_residual_var, input_flow_var)
            ...
            # adversarial loss
            loss_adv = criterion(validity, torch.cat((fake_var, valid_var), 0))

            # total loss.
            # since training D fixing G, no flow reconstruction mse loss
            loss = loss_cls * lr_cls + loss_adv * lr_adv_d

            #print(output.size())
            #print(validity.size())
            #print(torch.cat((target_var, target_var), 0).data.size())
            #print(torch.cat((fake_var, valid_var), 0).data.size())

            prec1, prec5 = accuracy(output.data, target, topk=(1, 5))
            acc_D_adv_batch = accuracy(validity.data, torch.cat((fake_var, valid_var), 0).data, topk=(1, ))
            losses.update(loss.data[0], output.size(0))
            losses_cls.update(loss_cls.data[0], output.size(0))
            losses_adv.update(loss_adv.data[0], validity.size(0))
            top1.update(prec1[0], output.size(0))
            top5.update(prec5[0], output.size(0))
            acc_D_adv.update(float(acc_D_adv_batch[0]), validity.size(0))
            #print(type(prec1[0]))
            #print(type(acc_D_adv_batch[0]))
            ...
            if i % PRINT_FREQ == 0:
                print(('D Epoch: [{0}][{1}/{2}], lr_gf: {lr:.7f}\t'
                       'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                       'Data {data_time.val:.3f} ({data_time.avg:.3f})\t'
                       'D Loss {loss.val:.4f} ({loss.avg:.4f})\t'
                       'D Loss_cls {loss_cls.val:.4f} ({loss_cls.avg:.4f})\t'
                       'D Loss_adv {loss_adv.val:.4f} ({loss_adv.avg:.4f})\t'
                       'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                       'acc_D_adv {acc_D_adv.val:.3f} ({acc_D_adv.avg:.3f})\t'
        ...
        if i % 2 == 1:
            ...
            # forward
            # For G, only feed generated samples for classification.
            if att == 0:
                output, validity, gen_flow = model(input_mv_var, input_residual_var)
            elif att == 1:
                output, validity, gen_flow, att_flow = model(input_mv_var, input_residual_var)
            ...
            loss_cls = criterion(output, target_var)
            # adversarial loss
            # For training G, only compute loss for fake
            loss_adv = criterion(validity, valid_var)
            ...
            # total loss
            loss = loss_cls * lr_cls + loss_adv * lr_adv_g + loss_mse * lr_mse

            prec1, prec5 = accuracy(output.data, target, topk=(1, 5))
            acc_G_adv_batch = accuracy(validity.data, valid_var.data, topk=(1, ))
            losses.update(loss.data[0], output.size(0))
            losses_cls.update(loss_cls.data[0], output.size(0))
            losses_adv.update(loss_adv.data[0], validity.size(0))
            losses_gf.update(loss_mse.data[0], output.size(0))
            top1.update(prec1[0], output.size(0))
            top5.update(prec5[0], output.size(0))
            acc_G_adv.update(float(acc_G_adv_batch[0]), validity.size(0))
            ...
            if i % PRINT_FREQ == 0:
                print(('G Epoch: [{0}][{1}/{2}], lr_gf: {lr:.7f}\t'
                       'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                       'Data {data_time.val:.3f} ({data_time.avg:.3f})\t'
                       'Loss {loss.val:.4f} ({loss.avg:.4f})\t'
                       'G Loss_cls {loss_cls.val:.4f} ({loss_cls.avg:.4f})\t'
                       'G Loss_adv {loss_adv.val:.4f} ({loss_adv.avg:.4f})\t'
                       'G Loss_mse {loss_mse.val:.4f} ({loss_mse.avg:.4f})\t'
                       'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                       'acc_G_adv {acc_G_adv.val:.3f} ({acc_G_adv.avg:.3f})\t'
                       'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                           epoch, i, len(train_loader),
                           batch_time=batch_time,
                           data_time=data_time,
...
def validate(val_loader, model, criterion, criterion_mse, lr_cls, lr_adv_g, lr_adv_d, lr_mse, att):
    # init meter
    batch_time = AverageMeter()
    losses_cls = AverageMeter()
    losses_gf = AverageMeter()
    ...
    top1 = AverageMeter()
    top5 = AverageMeter()
    acc_G_adv = AverageMeter()  # real/fake accuracy from the perspective of training G
