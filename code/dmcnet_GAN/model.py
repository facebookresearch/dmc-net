...
class Model(nn.Module):
    def __init__(self, num_class, num_segments, representation,
                 base_model='resnet152', new_length=1, use_databn=1, gen_flow_or_delta=0, gen_flow_ds_factor=0, arch_estimator='ContextNetwork', arch_d='Discriminator', att=0):
...
Initializing model:
    ...
    new_length:       {}.
        ...
        # part 3: the discriminator in GAN
    ...
    def _prepare_base_model(self, base_model):
        ...
        elif self.arch_estimator == 'DenseNet':
            self.gen_flow_model = EstimatorDenseNet(5)
        elif self.arch_estimator == 'DenseNetSmall':
            self.gen_flow_model = EstimatorDenseNetSmall(5)
        elif self.arch_estimator == 'DenseNetTiny':
            self.gen_flow_model = EstimatorDenseNetTiny(5)
        elif self.arch_estimator == 'DenseNetTinyEarlyFusionSum':
            self.gen_flow_model = EstimatorDenseNetTinyEarlyFusionSum(5)
        elif self.arch_estimator == 'DenseNetTinyEarlyFusionStack':
    ...
    def forward(self, input_mv, input_residual, input_flow=None):
        ...
        # Feed into classification model and discriminator (if needed)
        if input_flow is not None:
            input_flow = input_flow.view((-1, ) + input_flow.size()[-3:])
            input = torch.cat((gen_flow, input_flow), 0) # first fake then real
        else:
            input = gen_flow
        base_out = self.base_model(gen_flow)
        validity = self.discriminator(input)

        if self.att == 0:
            return base_out, validity, gen_flow
        elif self.att == 1:
            return base_out, validity, gen_flow, att_flow
